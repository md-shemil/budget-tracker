...
#         cursor.execute("""
#             INSERT INTO splits (sender_id, receiver_email, receiver_name, amount, description, status)
#             VALUES (%s, %s, %s, %s, %s, 'pending')
#         """, (user_id, email, name, amount, description))
#         db.commit()
#         flash('Split created successfully!', 'success')
#         return redirect(url_for('split'))

#     # Show only your splits
#     cursor.execute("SELECT * FROM splits WHERE sender_id = %s", (user_id,))
#     splits = cursor.fetchall()

#     return render_template('split.html', splits=splits)


#--------------------mark--paid------------for---the---above
# from datetime import datetime

# @app.route('/split/mark_paid/<int:split_id>', methods=['POST'])
...
#     # 1. Get the split details
#     cursor.execute("SELECT user_id, amount, description FROM split WHERE id = %s", (split_id,))
#     split = cursor.fetchone()

#     if split:
#         user_id, amount, description = split
#         now = datetime.now()

#         # 2. Mark the split as paid
#         cursor.execute("UPDATE split SET status = 'paid' WHERE id = %s", (split_id,))

#         # 3. Add an income transaction
#         cursor.execute("""
#             INSERT INTO transactions (user_id, type, amount, description, date)
#             VALUES (%s, 'income', %s, %s, %s)
#         """, (user_id, amount, f"Split received: {description}", now))
